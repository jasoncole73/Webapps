/* One-clock JS with extra-time killer and soft clouds retained */
const STORAGE_KEY='weather_flip_zip'; const REFRESH_MS=10*60*1000; const DEFAULT_ZIP='10001';
window.setupClock=function(t){const p=v=>String(v).padStart(2,'0'); const u=()=>{const n=new Date(); t.value=[p(n.getHours()),p(n.getMinutes()),p(n.getSeconds())]; const sr=document.getElementById('sr-time'); if(sr) sr.textContent=t.value.join(':'); killExtraTime(); }; u(); Tick.helper.interval(u,1000);};

function killExtraTime(){
  const wrap=document.querySelector('.clock-wrap'); if(!wrap) return;
  const guess = el => {
    const txt=(el.textContent||'').trim();
    return /^\d{1,2}:\d{2}:\d{2}$/.test(txt) && !el.closest('.tick');
  };
  wrap.querySelectorAll('*').forEach(el=>{ if(guess(el)) el.classList.add('hidden-by-fix'); });
}
...
function fxByWeather(main,night){
  if(night){ if(main==='Rain'||main==='Drizzle') fxRain(); else if(main==='Thunderstorm') fxStorm(); else if(main==='Snow') fxSnow(); else if(main==='Mist'||main==='Fog'||main==='Haze') fxFog(); else if(main==='Clouds') fxCloudsNight(); else fxMoon(); }
  else { if(main==='Rain'||main==='Drizzle') fxRain(); else if(main==='Thunderstorm') fxStorm(); else if(main==='Snow') fxSnow(); else if(main==='Mist'||main==='Fog'||main==='Haze') fxFog(); else if(main==='Clouds') fxCloudsDay(); else fxSun(); }
}
function drawSoftCloud(x,y,w,h,a){ ctx.save(); ctx.globalAlpha=a; ctx.filter='blur(2px)'; const r=h/2; const bumps=[{dx:-w*0.2,dy:0,rr:r*0.9},{dx:-w*0.05,dy:-r*0.4,rr:r*1.1},{dx:w*0.15,dy:-r*0.2,rr:r*1.0},{dx:w*0.35,dy:0,rr:r*0.9}]; ctx.fillStyle='rgba(255,255,255,0.95)'; bumps.forEach(b=>{ ctx.beginPath(); ctx.arc(x+b.dx,y+b.dy,b.rr,0,Math.PI*2); ctx.fill(); }); ctx.fillRect(x-w*0.35,y,w*0.7,r); ctx.restore(); }
...
function animate(){ requestAnimationFrame(animate); ctx.clearRect(0,0,canvas.width,canvas.height);
  ...
  if(['CloudsDay','CloudsNight','Rain','Snow','Fog','Storm'].includes(fxType)){ clouds.forEach(c=>{ drawSoftCloud(c.x,c.y,c.w,c.h,c.a); c.x += c.v; if (c.x - c.w*0.35 > canvas.width) c.x = -c.w*0.35; }); }
  if (fxType==='CloudsNight' || fxType==='Moon') drawMoon();
  if (fxType==='Rain' || fxType==='Storm'){ ctx.strokeStyle='rgba(180,200,255,.7)'; ctx.lineWidth=1.1; for (let i=0;i<140;i++){ const x=(i*17+Date.now()*0.35)%canvas.width; const y=(i*29+Date.now()*0.9)%canvas.height; ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(x+2,y+14); ctx.stroke(); } if(fxType==='Storm' && Math.random()<0.02){ ctx.fillStyle='rgba(255,255,255,.45)'; ctx.fillRect(0,0,canvas.width,canvas.height); } }
  if (fxType==='Snow'){ ctx.fillStyle='rgba(255,255,255,.95)'; for (let i=0;i<120;i++){ const x=(i*23+Math.sin((Date.now()+i*100)*0.002)*20+canvas.width)%canvas.width; const y=(i*31+Date.now()*0.15)%canvas.height; ctx.beginPath(); ctx.arc(x,y,1.4,0,Math.PI*2); ctx.fill(); } }
  if (fxType==='Fog'){ for (let i=0;i<6;i++){ const y=40+i*60+Math.sin((Date.now()+i*500)*0.0003)*10; const g=ctx.createLinearGradient(0,y,0,y+90); g.addColorStop(0,'rgba(255,255,255,0)'); g.addColorStop(0.5,'rgba(255,255,255,.14)'); g.addColorStop(1,'rgba(255,255,255,0)'); ctx.fillStyle=g; ctx.fillRect(0,y-25,canvas.width,90); } }
}
animate();
...
geoBtn.addEventListener('click', ()=>{ if(!navigator.geolocation){ setStatus('Geolocation not supported.','err'); return; } setStatus('Locating…','warn'); navigator.geolocation.getCurrentPosition(async p=>{ try{ const d=await fetchWeatherByCoords(p.coords.latitude,p.coords.longitude); applyWeather(d); setStatus('Updated for your location','ok'); } catch(e){ console.error(e); setStatus('Could not fetch weather.','err'); } }, err=> setStatus('Location permission denied.','err'), { enableHighAccuracy:false, timeout:8000, maximumAge:60000 }); });
