/* One-clock: global MutationObserver + PoP (chance of rain) */
...
/* Aggressive mirror killer: watch the whole app */
(function killMirrorsGlobal(){
  ...
  const hideIfTime = (el)=>{
    if(!el || el.closest('.tick')) return;
    if(isTimeText(el.textContent)){ el.style.display='none'; el.classList.add('digital-time'); }
  };
  // initial sweep
  ...
  const mo=new MutationObserver(muts=>{
    muts.forEach(m=>{
      if(m.type==='childList'){
        m.addedNodes.forEach(n=>{ if(n.nodeType===1){ hideIfTime(n); n.querySelectorAll&&n.querySelectorAll('*').forEach(hideIfTime);} });
...
/* Canvas FX (soft clouds) — same as earlier build (omitted for brevity in comments) */
...
function applyPoPFromForecast(forecast){
  try{
    ...
    // find the item within next ~4.5 hours (closest future)
