/* Weather-Flip JS with stricter ZIP handling and no PQINA credit */
...
function normalizedZip(raw){ return (raw||'').trim().replace(/\s+/g,''); }

async function fetchWeatherByZip(zip){ if(!window.WEATHER_API_KEY) throw new Error('NO_KEY'); const url=`https://api.openweathermap.org/data/2.5/weather?zip=${encodeURIComponent(zip)},US&appid=${window.WEATHER_API_KEY}&units=imperial`; const r=await fetch(url); if(!r.ok) throw await buildHttpError(r); return r.json(); }
async function fetchWeatherByCoords(lat,lon){ if(!window.WEATHER_API_KEY) throw new Error('NO_KEY'); const url=`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${window.WEATHER_API_KEY}&units=imperial`; const r=await fetch(url); if(!r.ok) throw await buildHttpError(r); return r.json(); }
async function buildHttpError(res){ const t=await res.text(); const e=new Error('HTTP_'+res.status); e.httpStatus=res.status; e.body=t; return e; }
...
function applyWeather(d){ const {name,main,weather,sys,dt,timezone}=d||{}; const w=(weather&&weather[0])||{};
  ...
  const night=isNight(w.icon,sys,dt,timezone); document.documentElement.classList.toggle('night',night); themeByWeather(night); fxByWeather(w.main,night);
}
...
function themeByWeather(night){ const root=document.documentElement; root.style.setProperty('--tile-bg',night?'#1b1f2a':'#2b2b2f'); root.style.setProperty('--tile-fg',night?'#e5e7eb':'#ffffff'); }
...
function fxMoon(){ fxType='Moon'; particles=[]; stars=Array.from({length:90},()=>({x:Math.random()*canvas.width,y:Math.random()*canvas.height*0.6,a:Math.random()*0.6+0.2,r:Math.random()*1.3+0.6})); lightningTimer=0; }
...
function animate(){ requestAnimationFrame(animate); ctx.clearRect(0,0,canvas.width,canvas.height);
  if(fxType==='Sun'){ const cx=160,cy=90,r=55; const g=ctx.createRadialGradient(cx,cy,10,cx,cy,r*3); g.addColorStop(0,'rgba(255,210,80,.9)'); g.addColorStop(1,'rgba(255,210,80,0)'); ctx.fillStyle=g; ctx.beginPath(); ctx.arc(cx,cy,r*3,0,Math.PI*2); ctx.fill(); ctx.strokeStyle='rgba(255,200,80,.55)'; ctx.lineWidth=3; const t=Date.now()*0.001; for(let i=0;i<12;i++){ const a=t+(Math.PI*2/12)*i, x1=cx+Math.cos(a)*r, y1=cy+Math.sin(a)*r, x2=cx+Math.cos(a)*(r+22), y2=cy+Math.sin(a)*(r+22); ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.stroke(); } }
...
/* Input validation that reacts immediately */
input.addEventListener('input', ()=>{
  const z = normalizedZip(input.value);
  if (ZIP_RE.test(z)) setStatus('', '');
});
...
geoBtn.addEventListener('click', ()=>{
  ...
  navigator.geolocation.getCurrentPosition(async p=>{
    try{ const d=await fetchWeatherByCoords(p.coords.latitude,p.coords.longitude); applyWeather(d); setStatus('Updated for your location','ok'); }
