/* Weather-Flip+ JS with geolocation, dark mode, themed tiles, and weather FX */
...
function themeByWeather(main, temp){
  ...
  // Adjust global theme + tile colors based on main condition
  const MAP = {
...
geoBtn.addEventListener('click', () => {
  ...
  navigator.geolocation.getCurrentPosition(async pos => {
    ...
    try {
      ...
      // Store nearest ZIP if present
      if (data?.name){ saveZip(data?.zip || loadZip()); }
